...
class DashPage {
    ...
    selectDay(appointmentDate){
        ...
        let today = new Date()
        let lastDayOfMonth = new Date(today.getFullYear(), today.getMonth() + 1)

        if(today.getDate() === lastDayOfMonth.getDate()){
            cy.log('Hoje é o ultimo dia dos mês!!')

            cy.get(el.nextMonthButton)
                .should('be.visible')
                .click()

            //Isso é um  checkpoint para garantir que houve troca do calendario
            let monthName 
